...
lasso_pred_plot <- ggplot(lasso_resid, aes(x = particles_l, y = .pred)) + geom_point() + 
  labs(title = "LASSO: Pred vs Outcomes", x = "Particles/L Outcome", y = "Particles/L Prediction")
lasso_pred_plot


#plot residuals vs predictions
lasso_resid_plot <- ggplot(lasso_resid, aes(y = .resid, x = .pred)) + geom_point() + 
  labs(title = "LASSO: Pred vs Residuals", x = "Particles/L Prediction", y = "Residuals")
...
tree_pred_plot <- ggplot(tree_resid, aes(x = particles_l, y = .pred)) + geom_point() + 
  labs(title = "Decision Tree: Pred vs Outcomes", x = "Particles/L Outcome", y = "Particles/L Prediction")
tree_pred_plot

#plot residuals vs predictions
tree_resid_plot <- ggplot(tree_resid, aes(y = .resid, x = .pred)) + geom_point() + 
  labs(title = "Decision Tree: Pred vs Residuals", x = "Particles/L Prediction", y = "Residuals")
...
The best decision tree has an RMSE of 102.2, which is better than the null model RMSE of 109. However, the decision tree did not perform as well well as the Lasso regularized model, which had an RMSE of about 100. 
...
set.seed(123)
#subset 3/4 of data as training set
data_split <- initial_split(complete, 
                            prop = 7/10, 
                            strata = particles_l) #stratify by MP concentration for balanced outcome
...
rf_pred_plot <- ggplot(rf_resid, aes(x = particles_l, y = .pred)) + geom_point() + 
  labs(title = "Random Forest: Pred vs Outcomes", x = "Particles/L Outcome", y = "Particles/L Prediction")
rf_pred_plot


#plot residuals vs predictions
rf_resid_plot <- ggplot(rf_resid, aes(y = .resid, x = .pred)) + geom_point() + 
  labs(title = "Random Forest: Pred vs Residuals", x = "Particles/L Prediction", y = "Residuals")
...
```{r}
model_grid <- grid.arrange(lasso_pred_plot, lasso_resid_plot, rf_pred_plot, rf_resid_plot, tree_pred_plot, tree_resid_plot, ncol = 3)

ggsave(model_grid, filename = here("results", "model_grid.png"))
```

I'll need to go back and edit this image later so that the titles aren't cut off and the axis scales match up properly so the plots look right. I might also want to add a 45 degree diagonal line to the outcomes vs predictions plot, and some other features to improve appearance. 