...
I am realizing that we can get more localized population levels by using census tracts, which are smaller levels of census geography. Let's try to use FIPS codes (which include tract info) to obtain population data that might be more informative. 
...
# FIPS to census tract population
```{r}
library(tidycensus)
#tidycensus::census_api_key("c02ef0638e60f9d5fbb45de5219821e3a25cd7d2", install = TRUE)
get_decennial("block", state = Georgia, variables = population, year = 2019)
```

Here, we receive an error message suggesting to use NHGIS data since the most recent small census geography data is not available through `get_decennial()`.

# Using NHGIS data for more recent block-level population counts
```{r}
#Downloaded data from http://NHGIS.org
...
tractpopdata <- c %>% select(-"ALUBE001.x", -"zip", -"population")
