...
Filters were visually inspected using a dissecting microscope with magnification ranging from [INSERT]. Microplastic particles were identified based on the following characteristic features: unnatural homogenous color, unnatural shape, homogenous texture, homogenous width (no tapered end)[@Hidalgo-Ruz2012]. Microplastic concentration (No/L) was calculated by dividing the total microplastic particle count by the sample volume. Duplicates (samples A and B for each site) were averaged to obtain a singular concentration value for each sample site. 
...
### Mapping
I created some maps for concentrations at each sample sites throughout the seasons (saved as html files "map_season.html") for now, but I am unsure of how best to display these maps in the manuscript word document output. 
```{r}
#mapview::knit_print.mapview(here("results", "map_spring.html"))
```
